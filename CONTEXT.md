...
- 当前源码版本：`7.11.10`（Phase 0-4 完成 + 手牌回放 + GTO 基线域 + 河牌证据域 + 复盘牌理参考概念层（DDoG 教学维度 + spot 应用层 + 概念自测 69 题多题制与错题集 + 图像复核收尾 + 分布镜头扩量 + CH11 收尾 + 深度维度）；`npm run check` 全绿待木下验收；版本改动明细见 CHANGELOG）。
