// [V7.11.1 修改] 复盘牌理参考种子：四步框架保留为导航（范围合法性 → 线语义 → 街牌效应 → 偏离解读）。
// [V7.11.2 修改] v3：每格 = 导航语 + 应用条目（applicationIds → conceptSeed.js 的 CONCEPT_APPLICATIONS，
//   带 DDoG/博客出处）+ 概念引用（conceptIds → CONCEPT_SEEDS）。牌理内容只存在于概念域，本文件不自写叙述。
// 纪律：格内内容全部有出处；空格不允许（契约测试断言每格至少 1 条应用）。
// street/scenario/role/question 为 spotMatcher 匹配字段，evidenceRefs 指向证据包 id，boundary 为证据转移边界——保持不变。
export var POKER_LOGIC_SEED_VERSION = 'v3';
...
export var POKER_LOGIC_SEEDS = [
  {
    ...
    steps: {
      scope: {
        note: '本 spot = BTN 开牌、BB 跟注后的翻牌首动。先问：双方范围各自是什么形态，你的优势从哪来。',
        conceptIds: ['concept-range-morphology', 'concept-nut-advantage'],
        applicationIds: ['app-btn1-scope-1', 'app-btn1-scope-2'],
      },
      lines: {
        note: '前置线语义：BB check 后行动。下注的本质是拿走对手 equity 的实现权；"对方先下注（donk）"不属于本 spot（雷达与牌理参考都不覆盖）。',
        conceptIds: ['concept-range-bet'],
        applicationIds: ['app-btn1-lines-1', 'app-btn1-lines-2'],
      },
      streets: {
        note: '街牌效应：把牌面当作"改变双方 equity 分布形状"的事件，再对照信号卡的牌面专项参考（若有）。',
        conceptIds: ['concept-equity-buckets', 'concept-backdoor-equity'],
        applicationIds: ['app-btn1-streets-1'],
      },
      deviation: {
        note: '偏离解读：先对照 Radar 同场景基线，再回到本格要点检查机制；本卡不给频率答案。',
        conceptIds: [],
        applicationIds: ['app-btn1-deviation-1'],
      },
    },
  ...
  },
  {
    ...
    steps: {
      scope: {
        ...
        conceptIds: ['concept-range-morphology', 'concept-nut-advantage'],
        applicationIds: ['app-sb3-scope-1'],
      },
      lines: {
        note: '线语义：你是先行动方，check 是默认可接受选项；下注尺寸应瞄准让对手特定手类无差别。',
        conceptIds: ['concept-indifference'],
        applicationIds: ['app-sb3-lines-1'],
      },
      streets: {
        note: '街牌效应：OOP 下注价值随牌面改变分布的方式变化；backdoor 可见度是你中段手实现 equity 的关键。',
        conceptIds: ['concept-equity-buckets', 'concept-backdoor-equity'],
        applicationIds: ['app-sb3-streets-1', 'app-sb3-streets-2'],
      },
      deviation: {
        note: '偏离解读：对照 Radar SBvsBB 基线（注意其正确基线本来就低于 BTNvsBB），再回本卡要点检查。',
        conceptIds: [],
        applicationIds: ['app-sb3-deviation-1'],
      },
    },
  ...
  },
  {
    ...
    steps: {
      scope: {
        ...
        applicationIds: ['app-co4-scope-1', 'app-co4-scope-2'],
      },
      lines: {
        note: '线语义：过牌是策略不是软弱——对手用位置实现 equity，你的下注需要在被 float 后仍有后续计划。',
        conceptIds: ['concept-indifference'],
        applicationIds: ['app-co4-lines-1'],
      },
      streets: {
        note: '街牌效应：静态面（你的范围优势大）与动态面（接近对称）由分布形状区分，策略整体随之切换。',
        conceptIds: ['concept-equity-buckets', 'concept-backdoor-equity'],
        applicationIds: ['app-co4-streets-1'],
      },
      deviation: {
        note: '偏离解读：对照 Radar COvsBTN 基线与自己的转牌行动分布；下注频率低于 IP 加注者是结构结果。',
        conceptIds: [],
        applicationIds: ['app-co4-deviation-1'],
      },
    },
  ...
  },
  {
    ...
    steps: {
      ...
      streets: {
        ...
        applicationIds: ['app-btn6-streets-1'],
      },
      deviation: {
        note: '偏离解读：空白河牌下注偏低 = 放弃收租窗口；惊悚面不重新定价是偏高侧的典型问题；对照 Radar 基线。',
        conceptIds: [],
        applicationIds: ['app-btn6-deviation-1'],
      },
    },
  ...
  },
  {
    ...
    steps: {
      scope: {
        ...
        conceptIds: ['concept-nut-advantage'],
        applicationIds: ['app-sb8-scope-1'],
      },
      lines: {
        note: '线语义：下注以小注为主（薄价值 / 阻挡）；大注需要真两极或关键 blocker。',
        conceptIds: ['concept-value-bluff-ratio'],
        applicationIds: ['app-sb8-lines-1', 'app-sb8-lines-2'],
      },
      streets: {
        note: '街牌效应：惊悚面 middling 牌失去价值下注资格；成花河牌过牌设陷阱（check 不总是示弱）。',
        conceptIds: ['concept-equity-buckets', 'concept-blockers'],
        applicationIds: ['app-sb8-streets-1'],
      },
      deviation: {
        note: '偏离解读：面对下注侧按"抓诈唬三问"走；对手有位置的河牌下注更两极，MDF 是下界不是目标。',
        conceptIds: ['concept-bluff-catching', 'concept-mdf-boundary'],
        applicationIds: ['app-sb8-deviation-1', 'app-sb8-deviation-2', 'app-sb8-deviation-3'],
      },
    },
