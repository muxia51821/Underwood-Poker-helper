...
**这是隐性关键约束**：如果有人把 CSP 移到内联脚本之前（或改成 HTTP 响应头下发），整个应用会静默白屏。目前没有任何自动化检查守护这个顺序——e2e 冒烟跑的是 dev server（dev 不注入 CSP），等于 CSP+内联组合从未被 CI 覆盖。2026-08-25 我已用真浏览器对两个线上入口补做了一次人工验证（均正常），但建议未来把冒烟目标换成生产构建预览（`vite preview`）以纳入回归保护。
...
`npm run check` = 临时目录生产构建 → parser/storage 契约测试 → Playwright 冒烟（dev server）→ 产物契约校验（输出 JSON 证据含 SHA-256）。它是这个架构的核心护栏：单文件产物无法靠"看目录"审查，所以把正确性前移到构建期断言。本次 Pages 故障恰好证明它的价值与盲区并存——检查本身有效拦住了坏产物，但 CI 环境缺 Playwright 浏览器导致闸门误报失败（已修复），而 dev-server 冒烟又覆盖不到生产特有行为（见第 2 节）。

## 6. 风险清单（按当前实际影响排序）

| # | 风险 | 影响 | 现状 |
|---|---|---|---|
| 1 | e2e 只测 dev 模式，CSP+内联顺序无自动化保护 | 高（一旦破坏即全站白屏，且 CI 不会报警） | 已人工验证一次；建议改用 `vite preview` 冒烟 |
